...
class UserController extends CommonController {
    ...
    public function ajax_rank_edit(){

        $userrankmodel = new UserRankModel();

        $userrankmodel->startTrans();

        try {
            for ($i = 1; $i <= 12; $i++) {

                $data = array(
                    'u_rank_money' => $_POST['r' . $i],
                );
                $where = 'u_rank_id = ' . $i;
                $userrankmodel->updataone($where, $data);

            }
            $userrankmodel->commit();
            die(json_encode(array('str' => 1, 'msg' => L('rank_detail_yes'))));
        }catch (Exception $e){
            $userrankmodel->rollback();
            die(json_encode(array('str' => 2, 'msg' => L('rank_detail_no'))));
        }
