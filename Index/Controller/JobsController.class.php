    ...
    public function releaseProject(){
        if(IS_POST){
            ...
            if (!$Item->validate($rules)->create()){
                ...
                $fuweek = strtotime('-1 week');

                $usernum = $usermodel->findone('user_intime > '.$fuweek,'count(*) num')['num'];

                $coinruleone = $coinrulemodel->findone('s_coinrule_id = 5','*');
                $jianmoney = ceil($usernum/$coinruleone['s_coinrule_num'])*$coinruleone['s_coinrule_setcoin'];
    ...
    public function ReleaseProfessional(){
        ...
        if(IS_POST){
            ...
            if (!$Professional->validate($rules)->create()){
                ...
                $coinruleone = $coinrulemodel->findone('s_coinrule_id = 6','*');
                $jianmoney = ceil($usernum/$coinruleone['s_coinrule_num'])*$coinruleone['s_coinrule_setcoin'];
