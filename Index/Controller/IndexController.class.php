...
use Index\Model\ShouOfferModel;
use Index\Model\UserModel;

use Index\Model\UserRankModel;
...
class IndexController extends CommonController {
    ...
    public function index(){
        ...
        $userranklist = $userrankmodel->findlist('','*');

        $shouoffermodel = new ShouOfferModel();

        $shouofferlist = $shouoffermodel->findlist('','*');

        session('returnurl', $_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);

        $offer=M("s_offer")->WHERE(array("s_offer_id"=>3))->find();

	$signflag = M("u_sign")->where([
			'user_id' => $this->userid,
			'date' => ["LIKE", "%".date("Y-m-d")."%"]
		])->find();

	$donateItem = M('s_donateitem')->where('status = 1')->order('sort asc')->limit(4)->select();
	$this->assign(array(
            'newslist' => $newslist,
	...
	$price = I('get.price');
        $offe=M("s_offer")->where(array('s_offer_id'=>2))->find();
