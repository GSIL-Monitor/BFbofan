...
class AjaxController extends CommonController
{
			...
			if (!$this->post('username')) {
				...
				if (!$userone) {
					...
					if (md5($this->post('password')) . $userone['user_password_joint'] == $userone['user_password']) {
						...
						$usermodel->updataone($where,$userdata);
	...
	public function ajax_registerfirst()
	{
		...
		if (IS_POST) {
			...
			} else if(!isset($_POST['passwordone']) || !preg_match("/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/", $this->post('passwordone'))){

				die(json_encode(array('str' => 3, 'msg' => '密码格式错误！')));
	...
	//facebook注册第一步
	public function ajax_registerfacebook()
	{

		if (IS_POST) {

			$usermodel = new UserModel();
			$where['user_mail'] = $this->post('facebookid');
			$userone = $usermodel->findone($where);
			if ($userone){
				session('userid', $userone['user_id']);
				$userdata = array(
				...
				session('register_facebookid', $this->post('facebookid'));
				session('register_type', 2);
				die(json_encode(array('str' => 2, 'msg' => '该用户还未完善资料，请先完善资料')));
			}
	...
	public function ajax_registersecond()
	{
		...
		if (IS_POST) {
			...
			if (!isset($_POST['name']) || $this->post('name') == '') {
				...
				if (session('register_name')&&session('register_tag')){
		...
		if (IS_POST) {
			if (session('register_type')==1) {
				...
				if (!session('register_mail')) {
					...
					$grouparr = explode(',',$_POST['groupstr']);
					...
					if ($userone) {
						...
						try {
							...
							foreach ($grouparr as $gkey => $gvalue){
								...
								$gdata = array(
										'crowd_member_cid'=>$gvalue,
										'crowd_member_uid'=>$res,
										'crowd_member_status'=>0,
										'crowd_member_logintime'=>date('Y-m-d H:i:s',time()),
									      );
								$crowdmembermodel->add($gdata);

								$crowdone = $crowdmodel->haveone('crowd_id = '.$gvalue,'*');
								$cdata = array(
										'crowd_peoplenum'=>$crowdone['crowd_peoplenum']+1
									      );
								$crowdmodel->updataone('crowd_id = '.$gvalue,$cdata);
			...
			}else if (session('register_type') == 2){

				if (!session('register_facebookid')) {

					die(json_encode(array('str' => 4, 'msg' => 'Facebook验证已过时')));
				} else if (!session('register_name')) {
					...
					$data = array();
					$data['user_mail'] = session('register_facebookid');
					$data['user_name'] = session('register_name');
					$data['user_tag'] = session('register_tag');
					...
					if ($userone) {
						die(json_encode(array('str' => 8, 'msg' => '该Facebook已有用户注册')));
					} else {
						$usermodel->startTrans();
						try {
							$res = $usermodel->add($data);
							...
							unset($_SESSION['register_facebookid']);
							unset($_SESSION['register_password']);
							unset($_SESSION['register_name']);
							unset($_SESSION['register_sex']);
							unset($_SESSION['register_tag']);
		...
		if (IS_POST) {
			...
			if (!isset($_POST['mail']) || !preg_match("/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i", $this->post('mail'))) {
				...
				try {
					...
					$message = "【NEW-WORLD】Hello!Your verification code is:" . $code;
	...
	public function ajax_createinterest(){
		...
		if (IS_POST) {
			...
			if (!isset($_POST['crowd_icon']) || $this->post('crowd_icon') == '') {
				...
				$crowdmodel = new CrowdModel();
				$ishaveonewhere = 'crowd_name = "'.$this->post('crowd_name').'" and crowd_secondmarks = "'.$this->post('second').'" and crowd_thirdmarks = "'.$this->post('third').'" and crowd_fourthmarks = "'.$this->post('four').'"';
				$crowdishaveone = $crowdmodel->where($ishaveonewhere)->find();
				if ($crowdishaveone){
					die(json_encode(array('str' => 7, 'msg' => 'The community has been created')));
				}
		...
		if (IS_POST) {
			...
			if (!isset($_POST['crowd_icon']) || $this->post('crowd_icon') == '') {
				...
				try{
					...
					$conditionmodel->add($conditiondata);
					...
					if (!$info || count($filelist) != count($info)) {
						...
						try{
							...
							if ($questionid){
								...
								//发布成功
								//扣除积分
								$crowd_size = M('u_crowd')->where(['crowd_id' => $this->post('cid')])->field('crowd_peoplenum')->find()['crowd_peoplenum'];
								$ruleModel = M('s_coinrules')->where([
										'type' => 4,
										'min' => ['egt', $crowd_size],
										'max' => ['elt', $crowd_size]
										])->find();
								if($ruleModel) {
									M('u_user')->where(['user_id' => $this->userid])->setDec('user_havecoin', $ruleModel['coin']);
								}
			...
			if (!isset($_POST['name']) || $this->post('name') == ''){
				...
				if ($_FILES) {
					...
					if (!$info || count($_FILES['img']['name']) != count($info)) {
						// 上传错误提示错误信息
						die(json_encode(array('str' => 0, 'msg' => 'The image or video size is too large. The limitation is 3M or 1G.')));
					} else {
						...
						try{
							...
							if ($this->post('posttype')==2){
								$needcoin = M('s_coinrules')->where([
										'min' => ['egt' ,$crowdone['crowd_peoplenum']],
										'max' => ['lt', $crowdone['crowd_peoplenum']],
										'type'=> 3
										])->find();
								$needcoin = $needcoin ? $needcoin['coin'] : 0;
								if (($this->post('reward')+$jianmoney)>$userone['user_havecoin']){
									die(json_encode(array('str' => 6, 'msg' => '积分不足')));
								}
								$userdata['user_havecoin'] = $userone['user_havecoin']-$this->post('reward')-$needcoin;
								$notedata['note_reward'] = $this->post('reward');
							}
							if ($this->post('posttype')==3){
								$notedata['note_reward'] = $this->post('reward')*2;
					...
					try{
						...
						if ($this->post('posttype')==2){
							...
							$coinruleone = $coinrulemodel->findone('s_coinrule_id = 2','*');
							$jianmoney = ceil($crowdone['crowd_peoplenum']/$coinruleone['s_coinrule_num'])*$coinruleone['s_coinrule_setcoin'];

							if (($this->post('reward')+$jianmoney)>$userone['user_havecoin']){
								die(json_encode(array('str' => 6, 'msg' => '积分不足')));
							}
							$userdata['user_havecoin'] = $userone['user_havecoin']-$this->post('reward')-$jianmoney;
	...
	public function ajax_noteZan(){
		...
		if (IS_POST){
			...
			if (!isset($_POST['commemtid'])||$this->post('commemtid')==''){
				...
				$notemodel = new NoteModel();
				$commentone = $notemodel->findone('note_id = '.$this->post('commemtid'));


				$uidlist = explode(',',$commentone['note_counr_zans']);
				if (in_array($this->userid,$uidlist)){

					die(json_encode(array('str' => 4,'msg'=>'你已经点赞过了')));
				}else{

					$data['note_counr_zan'] = $commentone['note_counr_zan']+1;


					if ($commentone['note_counr_zans']){
						$data['note_counr_zans'] = $this->userid;
					}else{
						$data['note_counr_zans'] = ','.$this->userid;
					}


					$res = $notemodel->updataone('note_id = '.$this->post('commemtid'),$data);

					if ($res){
						//累计点赞数
						$notemodel->where(['note_id' => $this->post('commemtid')])->setInc('note_zan_count', 1);
						$note_zan_count = ++$commentone['note_zan_count'];
						$ruleModel = M('s_coinrules')->where('type = 2 and min = '.$note_zan_count)->find();
						if($ruleModel) {
							M('u_user')->where(['user_id' => $commentone['note_uid']])->setInc('user_havecoin', $ruleModel['coin']);						
						}

						$commenttwo = $notemodel->findone('note_id = '.$this->post('commemtid'));
	...
	public function ajax_notentUnzan(){
			...
			if (!isset($_POST['money'])||$this->post('money')==''){
				...
				try {
					...
					$userdata = array(
							...
							'user_outcoin' => $userone['user_outcoin'] + $this->post('money'),
							);
					...
					if ($res&&$noteres) {
						$usermodel->commit();
						M('u_user')->where(['user_id' => $noteone['note_uid']])->setInc('user_havecoin', $this->post('money'));
						die(json_encode(array('str' => 1, 'msg' => '正在下载')));
					} else {
						$usermodel->rollback();
	...
	public function ajax_createDemand(){
		...
		if (IS_POST){
			...
			if (!isset($_POST['cid'])||$this->post('cid')==''){
				...
				$coinruleone = $coinrulemodel->findone('s_coinrule_id = 4','*');
				$jianmoney = ceil($crowdone['crowd_peoplenum']/$coinruleone['s_coinrule_num'])*$coinruleone['s_coinrule_setcoin'];

				if ($jianmoney>$userone['user_havecoin']){
					die(json_encode(array('str' => 6, 'msg' => '积分不足')));
				}
				$userdata['user_havecoin'] = $userone['user_havecoin']-$jianmoney;
				...
				$status=substr($str,0,strlen($str)-1);
				...
				$data = array(
						...
						'tutorship_issue_status'=>$status,
						'tutorship_issue_datetime'=>$this->post('datetime')
					     );
	...
	public function ajax_createCounseling(){
		...
		if (IS_POST){
			...
			if (!isset($_POST['cid'])||$this->post('cid')==''){
				...
				$coinruleone = $coinrulemodel->findone('s_coinrule_id = 3','*');
				$jianmoney = ceil($crowdone['crowd_peoplenum']/$coinruleone['s_coinrule_num'])*$coinruleone['s_coinrule_setcoin'];
				...
				$data = array(
						...
						'tutorship_issue_status'=>$status,
						'tutorship_issue_datetime'=>$this->post('dat_time')
				...
				$tutorissuemodel->startTrans();
				try {
					$res = $tutorissuemodel->add($data);


					$usermodel->updataone('user_id = '.$this->userid,$userdata);
					$tutorissuemodel->commit();
					die(json_encode(array('str' => 1, 'id' => $res, 'cid' => $this->post('cid'))));
				}catch (Exception $e){
					$tutorissuemodel->rollback();
					die(json_encode(array('str' => 2,'msg'=>'发布失败')));
				}
